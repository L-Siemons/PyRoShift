...
from pyroshiftGA import fiIeIO as IO
...
def First_Gen(Sim_number_iteration, params):
    ...
    cdef float Random_Sum = 0.0
    cdef float RandomFloat = 0.0
    cdef int indiv
...
def select_parents(daughter_generation, probabilities, params):
    ...
    cdef int i
    cdef int indiv
    cdef float parent_selector
    cdef int counting_along_for_parent
...
def mate_and_mutate(parent_list, parent_generation, MeanResults, Measured_shifts, params):
    ...
    cdef int individual_counter = 0
    cdef int parent0
    cdef int parent1
    cdef int i
    cdef float break_point
    cdef float mutation_deicder
    cdef int state_decider
    cdef int family_member
    cdef int item
    cdef int number_of_states = params.number_of_states

    new_generation = {}


    #here we use a single break point which is randomly chosen
    for parent0, parent1 in parent_list:

        current_individual = []
        break_point = rn.randint(0,number_of_states-1)
        for i in range(number_of_states):
            if i <= break_point:
                current_individual.append(parent_generation[parent0][i])
            else:
                current_individual.append(parent_generation[parent1][i])
        ...
        #most fit parent
        family = {}
        family[0] = parent_generation[parent0]
        family[1] = parent_generation[parent1]
